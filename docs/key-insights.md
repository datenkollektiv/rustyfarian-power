...
- **Heltec V3 charge-controller IC marking and STAT/CHRG GPIO are still unconfirmed:** The charge *behaviour* is confirmed (see above), but Heltec does not publicly disclose the exact charge-controller part number, and no STAT/CHRG status GPIO is documented — both must be verified against the V3 schematic before an `EspChargingMonitor` can be written for that board.
  The charger IC sits on the back behind the reset switch and runs hot while charging a depleted cell (normal; thermally limited to ~100 °C).
  `hardware-setup.md` hints at a "BQ"/TI-series part — plausible but unverified.
